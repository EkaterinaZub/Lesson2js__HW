"Use strict";

// task 1
// Создайте переменные: name, age, city, phone, email, company; и получите через ф-цию
// prompt() соответствующие значения: ваше имя, возраст, город проживания и т.д.
// Выведите на экран фразу: «Меня зовут %Имя%. Мне %Возраст% лет. Я проживаю в
// городе %Город% и работаю в компании %Компания%. Мои контактные данные:
// %Телефон%, %Почта%.».



let name = prompt('Как Вас зовут?');
age = prompt('Сколько Вам лет?');
{


    city = prompt('Где Вы проживаете?');
    company = prompt('В какой компании Вы работаете?');
    phone = prompt('Ваш номер телефона?');
    email = prompt('Ваш адрес электронной почты?');

    console.log("Меня зовут " + name);
    console.log("Мне " + age + " лет");
    console.log("Я проживаю в " + city);
    console.log("Я работаю в компании " + company);
    console.log("Мой номер телефона " + phone);
    console.log("Мой адрес электронной почты " + email);

}

// task 2
// Определите по введенному возрасту (исп. значение из задания 1) год рождения.
// Выведите на экран «%Имя% родился в %Год% году.».

{
    let yearCalculation = (2023 - age)
    console.log(name + " родился в " + yearCalculation);
}

// task 3
// Дана строка из 6-ти цифр. Проверьте, что сумма первых трех цифр равняется сумме
// вторых трех цифр. Если это так - выведите 'да', в противном случае выведите 'нет'.

{
    let string = "456356";
    sumstring = Number(string[0]) + Number(string[1]) + Number(string[2]);
    sumstring2 = Number(string[3]) + Number(string[4]) + Number(string[5]);

    if (sumstring === sumstring2) {
        console.log("да");
    }
    else {
        console.log("Нет")
    }

}

//  task 4
// Если переменная a больше нуля, то выведите 'Верно', иначе выведите 'Неверно'.
// Проверьте работу скрипта при a, равном 1, 0, -3.

{
    let a = 5;

    if (a > 0) {
        console.log("Верно");
    } else {
        console.log("Неверно");
    }
}

//  task 5
// Создайте переменные a=10 и b=2. Выведите их сумму, разность, произведение,
// частное, а также, если сумма этих чисел больше 1, то возведите полученную сумму в
// квадрат.

let a = 10;
b = 2;
{

    console.log(a + b);
    console.log(a - b);
    console.log(a * b);
    console.log(a / b);

    if ((a + b) > 1) {
        console.log((a + b) ** 2);
    }
}

// task 6
// Если переменная a (из задания 5) больше 2 и меньше 11, или переменная b (из задания
//  5) больше или равна 6 и меньше 14, то выведите 'Верно', в противном случае выведите
//  'Неверно'.

{
    if (a > 2 && a < 11 || b >= 6 && b < 14) {
        console.log("Верно");
    }
    else {
        console.log("Неверно");
    }
}

//  task 7
// В переменной n лежит число от 0 до 59. Определите в какую четверть часа попадает
// это число (в первую, вторую, третью или четвертую).

{
    let n = 11;




    if (n >= 1 && n <= 15) {
        console.log("первая четверть часа");
    }
    else if (n > 15 && n <= 30) {
        console.log("вторая четверть часа");
    }
    else if (n > 30 && n <= 45) {
        console.log("третья четверть часа");
    }
    else if (n > 45 && n <= 59) {
        console.log("четвертая четверть часа");
    }

}

//  task 8
// В переменной day лежит число из интервала от 1 до 31. Определите в какую декаду
// месяца попадает это число (в первую, вторую или третью).

{
    let day = 23;

    if (day >= 1 && day <= 10) {
        console.log("первая декада месяца");
    }
    else if (day > 10 && day <= 20) {
        console.log("вторая декада месяца");
    }
    else if (day > 20 && day <= 31) {
        console.log("третья декада месяца");
    }

}

// task 9
// Напишите скрипт, который переводит дни в года (условно 1г = 365дн), в месяцы
// (условно 1м = 31день), в недели, в часы, в минуты и в секунды. Дробные результаты
// вычислений принимаются. Если кол-ва дней не хватает до полного года, месяца,
// недели, вывести сообщение «Меньше года», «Меньше месяца» и «Меньше недели»,
// соответственно.

let day =300;
{
    let day = 460;
    year = day / 365;
    month = day / 31;
    aWeek = day / 7;
    hour = day * 24;
    minute = day * 1440;
    second = day * 86400;


    if (year > 1) {
        console.log(year.toFixed(2) + " года");
    }
    if (day < 365) {
        console.log("Меньше года");
    }
    if (month > 1) {
        console.log(month.toFixed(2) + " месяца");
    }
    if (day < 31) {
        console.log("Меньше месяца");
    }

    if (aWeek > 1) {
        console.log(aWeek.toFixed(2) + " недели");
    }
    if (day < 7) {
        console.log("Меньше недели");
    }
    if (hour > 1) {
        console.log(hour + " часа");
    }
    if (minute > 1) {
        console.log(minute + " минуты");
    }

    if (second > 1) {
        console.log(second + " минуты");
    }


}

// task 10
// Напишите скрипт, который по введенному дню (исп. значение переменной из 8
//     задания) в году (например, 256) определит месяц (от 1 до 12) и пору года (зима, лето и
//     т.д.). Скрипт определение поры года написать через switch.
{
    let season = (day >= 1 && day <= 59 || day >= 335 && day <= 365) ? 1 :  "" || (day >= 60 && day <=151) ? 2 : "" || (day >= 152 && day <= 243) ? 3 : "" || (day >= 244 && day <=334) ? 4 : " ";
        
        console.log(season);

    if (day >= 1 && day <= 31){
        console.log(" месяц 1 январь");
    } 
    if (day >= 32 && day <= 59) {
        console.log(" месяц 2 февраль");
    }
    if (day >= 60 && day <= 90) {
        console.log(" месяц 3 март");
    }
    if (day >= 91 && day <= 120) {
        console.log(" месяц 4 апрель");
    }
    if (day >= 121 && day <= 151) {
        console.log(" месяц 5 май");
    }
    if (day >= 152 && day <= 181) {
        console.log(" месяц 6 июнь");
    }
    if (day >= 182 && day <= 212) {
        console.log(" месяц 7 июль");
    }
    if (day >= 213 && day <= 243) {
        console.log(" месяц 8 август");
    }
    if (day >= 244 && day <= 273) {
        console.log(" месяц 9 сентябрь");
    }
    if (day >= 274 && day <= 304) {
        console.log(" месяц 10 октябрь");
    }
    if (day >= 305 && day <= 334) {
        console.log(" месяц 11 ноябрь");
    }
    if (day >= 335 && day <= 365) {
        console.log(" месяц 12 декабрь");
    }


    
    switch (season) {
        case 1 :
            console.log("Зима");
            break;
        case 2 :
            console.log("Весна");
            break;   
         case 3 :
            console.log("Лето");
            break;   
        case 4 :
            console.log("Осень");
            break;   
    }



}